...
If any function is missing a positive/negative case, input, or output description, mark that function as `Missing Parameters` and ask the user to provide the missing parameters. Do not invent business inputs or expected outputs.

Coverage checks apply only to non-generated `.cs` files/functions changed in the tag-to-branch diff. Tests for unchanged functions, non-`.cs` files, or generated `.cs` files may exist, but they must not be counted as coverage for the report gate.

Do not create tests for generated C# code, including Connected Services, Service References, `Reference.cs`, `.g.cs`, `.designer.cs`, `.generated.cs`, files containing `<auto-generated>`, or files marked with `GeneratedCodeAttribute`.

If the diff contains no testable `.cs` changes, do not create placeholder test cases. The report test items should remain empty.
