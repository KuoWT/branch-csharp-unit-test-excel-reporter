...
只有副檔名為 `.cs` 且不是自動產生碼的改動會產生或檢查單元測試，其餘副檔名會忽略。

像 Connected Services、Service References、`Reference.cs`、`.g.cs`、`.designer.cs`、`.generated.cs`、含 `<auto-generated>` 或 `GeneratedCodeAttribute` 的 C# 檔案，都會視為自動產生碼，不會產生單元測試。

如果沒有可測試的 `.cs` 改動，測試項目會是空的，不會產生假的測試項目。
